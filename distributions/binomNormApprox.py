...
if __name__ == '__main__':
    ...
    print('Normal approximation of P(X <= {0}) = P(X <= {1}) ~= P(Z <= {2:.2}) = {3:.8}'\
        .format(x, x + 0.5, zval, probcum))
    print('')
    ...
    print('P(X = {0}) ~= P({1} <= X <= {2}) ~= P({3:.2} <= Z <= {4:.2}) = {5:.8} ~= {6:.8}'\
        .format(x, x - 0.5, x + 0.5, zval1, zval2, prob1, prob2))
