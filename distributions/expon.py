...
if __name__ == '__main__':
    ...
    print('{0}The probability that event E will occur in {1:.2} <= x <= {2:.2} units is {3:.8}'\
        .format(space, x1, x2, prob))

    p = 0.9
    x = stats.expon.ppf(1 - p, 0, 1 / lamb)
    assert(0.00421 - round(x, 5) == 0)
    print('{0}The the interval that event E will NOT occur with a probability of {1:.4} is {2:.4}'\
        .format(space, p, x))
    ...
    print('{0}The probability of event E occuring in <= {1:.2} units is {2:.3}'\
        .format(space, x, prob))
